...
const { tokens } = require("./config").jwt;
...
app.post("/login", urlencodedParser, async (req, res) => {
    ...
    if (!bcrypt.compareSync(req.body.password, resultFind[0]["password"])) {
        res.redirect("/login");
        return
    }
    else {
...
app.get("/logout", jwt.CheckJwt, async (req, res) => {
    await blackList.AddBlackList(req.body[0]["username"], req.cookies[tokens.refresh.type])
    res.cookie(config.jwt.tokens.refresh.type, {}, { expires: new Date(Date.now()), maxAge: 0, path: "/refresh-token", })
    res.cookie(config.jwt.tokens.refresh.type, {}, { expires: new Date(Date.now()), maxAge: 0, path: "/logout", })
